    ...
    for res in results:
        if not res or not res.get('report'): 
            summary_data.append({
                ...
                "Score": "N/A", 
                "Thesis": "ERROR: Analysis failed to complete."
            })
        else: 
            report = res['report']
            summary_data.append({
    ...
    if successful_results:
        ...
        for i, res in enumerate(successful_results):
            with tabs[i]:
                ...
                col1.metric(
                    label=f"Return ({start_date} to {end_date})", 
                    value=f"{res['finance_analysis'].get('period_return_pct', 0):.2f}%"
                )
                col2.metric(
                    label="Largest Single-Day Move", 
                    value=f"{res['finance_analysis'].get('largest_daily_move_pct', 0):.2f}%"
                )
                ...
                if not news_data: 
                    st.info(f"No news articles found.")
